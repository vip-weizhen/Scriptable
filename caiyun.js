...
class Widget extends DJG {
  ...
  async renderMedium2(w){
    ...
    dateStack.addSpacer()
    const dateStr = this.getDateStr(currentDate, "M月d日  EEE")  
    // 农历信息
    ...
    for(let i = 0; i < 8; i++){
      ...
      if(i == 3){
        ...
        this.addText(updateStack, `${this.getDateStr(new Date(), "HH:mm")}更新`, 11, {opacity:0.8, lineLimit:1})
      }
  ...
  async setLeftDraw(leftDrawing, data){
    ...
    // 获取农历信息
    let lunarJoinInfo = await this.calendarDate()
    const lunar = lunarJoinInfo.lunarMonthCn+lunarJoinInfo.lunarDayCn;
    this.drawText(leftDrawing, 120, 5, 280, 28, currentDate.getFullYear().toString().concat("年").concat(dateFormat), "regular",25,"left")
    this.drawText(leftDrawing, 120, 36, 300, 28, lunarJoinInfo.weekday + "  " + lunar, "regular",25,"left") 
    ...
    // 日历事件
    let futureEvents = [];
    try{futureEvents = await CalendarEvent.between(currentDate, endDate, [])}catch(e){}
    let flag = false;
    let index = 0;
    if(!futureEvents[0]) {
      logError("日历权限获取失败！！")
      flag = true;
      futureEvents = await this.httpGet("https://gitee.com/scriptxx_djg/script/raw/master/script/futureEvents.js");
    ...
    for (let i = index; i < (3 + index); i++) {
      ...
      if(flag) eventColor = new Color(eventColor.hex);
      this.fillRect(leftDrawing, 12, 82+(i-index)*61, 5, 48, 2, eventColor)
      // 标题
      const title = event.title
      this.drawText(leftDrawing, 29, 80+(i-index)*62, 305, 26, title, "bold", 23, "left")
      ...
      this.drawText(leftDrawing, 29, 85+(i-index)*61+25, 305, 30, timeText, "medium", 20, "left", eventTimeColor);
    }
  }
...
tempHeight = (Math.round(data[i*2].value)-Math.round(dailyTemperature[0].min))*32/(Math.round(dailyTemperature[0].max)-Math.round(dailyTemperature[0].min))+8
  ...
  provideGreeting(date, lunarDate) {
      // 农历问候语
      const lunarGreetingText = this.widgetConfigs.lunarText[`${lunarDate}`]
      // 月份
      ...
      // 纪念日子
      let anniversary = this.widgetConfigs.anniversaryText[`${month}-${day}`]
  ...
  Run(){
    if (config.runsInApp) {
      ...
      this.registerAction("自动定位", async () => {
        await this.setCustomAction("自动定位刷新间隔", "建议刷新时间不低于10，单位：分钟", {
            locTime: '',
        }, false);
